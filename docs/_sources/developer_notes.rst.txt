...
Models instantiate with the ``load`` method have their attribute
automatically 'tracked' by the schema. For example,
...
To see more information about serialization/deserialization, see
Serialization/Deserialization below
...
Relationships are special model fields that establish One-to-One,
...
Trident will always attempt to *fullfill* relationships for attributes
if possible
...
The HasMany relationship defaults to the 'where\_callback' method. In
the HasMany ``__init__`` definition, params are established such that
