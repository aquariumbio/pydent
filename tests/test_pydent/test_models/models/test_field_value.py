...
def test_constructor_with_value(fake_session):
    fv = fake_session.FieldValue(
        ...
        value=400,
        field_type=fake_session.FieldType(choices="400", parent_class="OperationType"),
    )
    assert fv.value == 400


def test_raises_exception_with_wrong_choice(fake_session):
    fv = fake_session.FieldValue.load(
        dict(
            role="input",
            parent_class="Operation",
            value=400,
            field_type=dict(choices="400,301", id=5),
        )
    )
...
def test_constructor_with_container(fake_session):
    ...
    fv_with_container = fake_session.FieldValue.new(
        container=fake_session.ObjectType.load({"id": 5}),
        item=fake_session.Item.load({"id": 4, "object_type_id": 5}),
    )
    assert fv_with_container.object_type.id == 5
    with pytest.raises(AquariumModelError):
        fake_session.FieldValue.new(
            container=fake_session.ObjectType.load({"id": 5}),
            item=fake_session.Item.load({"id": 4, "object_type_id": 4}),
        )
...
def test_set_item(fake_session):
    ...
    fake_item = fake_session.Item.load(
        {"id": 300, "object_type": {"id": 3, "name": "tube"}}
    )
...
def test_set_sample_and_item_no_aft(fake_session):
    ...
    fake_item = fake_session.Item.load(
        {"id": 55, "object_type": {"id": 44, "name": "MySampleType"}}
    )
    fake_sample = fake_session.Sample.load(
        {"id": 3, "sample_type": {"id": 2, "name": "MySampleType2"}}
    )
...
def test_compatible_items(monkeypatch, fake_session):
    ...
    sample = fake_session.Sample.load({"id": 5, "name": "plasmid", "sample_type_id": 2})
