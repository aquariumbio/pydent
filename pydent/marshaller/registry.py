...
from pydent.marshaller.exceptions import SchemaModelException, SchemaException


class SchemaRegistry(type):
    """Stores a list of models that can be accessed by name."""

    _schemas = {}  # the registry of Schemas instantiated
    ...
    def __init__(cls, name, bases, selfdict):
        ...
        if bases != () and name != 'DynamicSchema':
            if name in SchemaRegistry._schemas:
                raise SchemaException("Cannot register '{}' since it already exists.".format(name))
            SchemaRegistry._schemas[name] = cls
    ...
    def grouped_fields(cls):
        ...
        for fname, field in cls.fields.items():
            ...
            for b in mro[:-2]:
                grouped[b.__name__].append((fname, field))
        return grouped
    ...
    def get(name):
        ...
        if name not in SchemaRegistry._schemas:
            raise SchemaException("Schema \"{}\" not found in ModelRegistry.".format(name))
        else:
            return SchemaRegistry._schemas[name]
...
class ModelRegistry(type):
    ...
    def get_model(name):
        ...
        if name not in ModelRegistry.models:
            raise SchemaModelException("Model '{}' not found in registry".format(name))
