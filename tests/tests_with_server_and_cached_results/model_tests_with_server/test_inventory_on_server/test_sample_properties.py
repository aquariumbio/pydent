...
def test_create_with_integer_properties(session):
    ...
    yeast = session.Sample.new(
        sample_type_id=yeast_strain.id, properties={"QC_length": 100}
    )
...
def test_create_with_sample_properties(session):
    ...
    yeast = session.Sample.new(
        sample_type_id=yeast_strain.id, properties={"Parent": parent}
    )
...
def test_create_with_sample_property_raises_exception(session):
    ...
    with pytest.raises(Exception):
        yeast = session.Sample.new(
            sample_type_id=yeast_strain.id, properties={"Parent": 1}
        )
...
def test_create_raises_exception_when_property_is_not_found(session):
    ...
    with pytest.raises(Exception):
        yeast = session.Sample.new(
            sample_type_id=yeast_strain.id, properties={"Not a field value name": 1}
        )
...
def test_create_raises_exception_with_wrong_choice(session):
    ...
    with pytest.raises(AquariumModelError):
        yeast = session.Sample.new(
            sample_type_id=yeast_strain.id,
            properties={"Mating Type": "not a mating type"},
        )
...
def test_create_with_choice(session):
    yeast_strain = session.SampleType.find_by_name("Yeast Strain")
    yeast = session.Sample.new(
        sample_type_id=yeast_strain.id, properties={"Mating Type": "MATa"}
    )
...
def test_update_properties(session):
    ...
    yeast = session.Sample.new(
        sample_type_id=yeast_strain.id,
        properties={"QC_length": 200, "Comp_cell_limit": "Yes"},
    )
    parent = session.Sample.one(query={"sample_type_id": yeast_strain.id})
...
class TestUpdateFieldValueArrays:
    ...
    def get_samples(self, session, example_sample_type):
        ...
        func = functools.partial(
            session.Sample.last, query=dict(sample_type_id=aft.sample_type_id)
        )
        return func
    ...
    def test_local_changes(
        ...
        assert (
            len(example_sample.field_value_array(self.FIELD_TYPE_NAME))
            == num_field_values
        )
        ...
        if test_server_changes:
            ...
            fvs_from_server = session.FieldValue.where(
                {
                    "parent_id": example_sample.id,
                    "name": self.FIELD_TYPE_NAME,
                    "parent_class": "Sample",
                }
            )
...
def test_update_properties_using_array(session, num_field_values):
    ...
    if num_field_values == 0:
        ...
        fragments = session.Sample.last(
            num_field_values, query=dict(sample_type_id=fragment_type.id)
        )

    # update the sample properties with fragments
    sample.update_properties({"Fragment Mix Array": fragments, "Length": 1000})
    ...
    new_set_fragments = session.Sample.last(
        10 - num_field_values, query=dict(sample_type_id=fragment_type.id)
    )
    sample.update_properties({"Fragment Mix Array": new_set_fragments, "Length": 1100})
