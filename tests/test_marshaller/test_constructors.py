...
from pydent.marshaller.exceptions import SchemaException, ModelValidationError, SchemaRegistryError
...
class TestSchemaConstructors:
    ...
    def test_has_model_schema_with_correct_identity(self, MyFirstModel, MySecondModel, MyControlModel):
        assert not MyControlModel.model_schema
    ...
    def test_get_from_schema_registry(self, MyFirstModel, MySecondModel, MyControlModel):
    ...
    def test_different_schemas(self, MyFirstModel, MySecondModel, MyControlModel):
        assert SchemaRegistry.get_model_schema('MyFirstModel') is MyFirstModel.model_schema
        assert SchemaRegistry.get_model_schema('MySecondModel') is MySecondModel.model_schema
...
class TestRelationshipConstructors:
    ...
    def test_standard_relationship_constructor(self, base):
        ...
        @add_schema
        class MyModel(base):
            fields = dict(field1=None, field2=None, field3=Relationship(MyOtherModel, 'find'))
...
def make_function(num_args, num_kwargs):
    ...
    if num_kwargs is None:
        ...
        kwargs = ["{}=None".format(chr(i + len(args) + ord('a'))) for i in range(num_kwargs)]

    exec('''
def foo({args}):
    print(locals())
'''.format(args=', '.join(args + kwargs).strip())
         )
    print(', '.join(args + kwargs))
    return (eval("foo"), ', '.join(args + kwargs))


def make_function_param(num_args, num_kwargs, num_callback_args, num_callback_kwargs, result, suffix="", valid_kwargs=True):
    ...
    return pytest.param(func, args_to_send, kwargs_to_send, result, id="callback({}) <- {}  {}".format(signature, sent, suffix))
...
class TestCallback:
    ...
    @pytest.mark.parametrize("callback, args, kwargs, expected", [
        ...
        make_function_param(1, 1, 0, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
        make_function_param(2, 1, 1, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
        make_function_param(3, 1, 2, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
    ...
    @pytest.mark.parametrize("callback, args, kwargs, expected", [
        ...
        make_function_param(0, 1, 0, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
        make_function_param(1, 1, 1, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
        make_function_param(2, 1, 2, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
    ...
    def test_lambda_callback(self, base, callback, args, kwargs, expected):
        ...
        def make_model():
            ...
            @add_schema
            class MyModel(base):
                fields = dict(field1=None, field2=None, field3=Callback(callback, args, kwargs))
    ...
    @pytest.mark.parametrize("callback, args, kwargs, expected", [
        ...
        make_function_param(2, 1, 0, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
        make_function_param(3, 1, 1, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
        make_function_param(4, 1, 2, 1, False, valid_kwargs=False, suffix="invalid kwargs"),
    ...
    def test_instance_relationship(self, base, callback, args, kwargs, expected):
        ...
        def make_model():
            ...
            @add_schema
            class MyModel(base):
                fields = dict(field1=None, field2=None, field3=Relationship(base, 'find', args, kwargs))
